...
SYSTEMCTL_APP:=isucondition.go.service

# TODO: nginxのファイルの場所を指定する
NGINX_CONF:=$(APP_HOME)/nginx/nginx.conf
NGINX_APP_CONF:=$(APP_HOME)/nginx/isucondition.conf
...
SLACKCAT_RAW_CMD=/home/isucon/notify_slack
...
# echo-branch: git rev-parse --abbrev-ref HEAD | $(SLACKCAT_RAW_CMD) -tee --stream

echo-branch:
	git rev-parse --abbrev-ref HEAD | $(SLACKCAT_RAW_CMD)
